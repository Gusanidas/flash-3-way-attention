...
def _tritt_bwd_dv2(
    ...
    dv2_block = tl.zeros([HEAD_DIM_pow2, BLOCK_SIZE_KV], dtype=tl.float32)
    ...
    dv2_block_ptr = (
        dv2
        + qvk_offset
        + tl.arange(0, HEAD_DIM)[:, None] * stride_dv2_dim
        + kv2_seq_offs[None, :] * stride_dv2_seq
    )
    tl.store(dv2_block_ptr, dv2_block, mask=mask_kv2[None, :] & mask_head_dim[:, None])
