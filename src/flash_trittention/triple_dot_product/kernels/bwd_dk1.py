...
def _tritt_bwd_dk1(
    ...
    v1_block = tl.load(
        v1_block_ptr, mask=mask_kv1[:, None] & mask_head_dim[None, :], other=0.0
    )
    if convert_to_float32:
        v1_block = v1_block.to(tl.float32)

    dk1_block = tl.zeros([HEAD_DIM_pow2, BLOCK_SIZE_KV], dtype=tl.float32)
    ...
    for start_kv_2 in range(lo_k2, hi_k2, BLOCK_SIZE_KV):
        ...
        for start_q in range(lo_q, hi_q, BLOCK_SIZE_Q):
            ...
            m_i = tl.load(M + offset, mask=m_mask, other=-masking_value)
            ...
            QK2 = tl.reshape(
                tl.trans(Q_block)[:, :, None] * k2_block[:, None, :],
                (HEAD_DIM_pow2, BLOCK_SIZE_Q * BLOCK_SIZE_KV),
            )
            dk1_block += tl.dot(QK2, dS_reshaped)
    ...
    dk1_block_ptr = (
        dk1
        + qvk_offset
        + kv1_seq_offs[None, :] * stride_dk1_seq
        + tl.arange(0, HEAD_DIM)[:, None] * stride_dk1_dim
    )
    tl.store(dk1_block_ptr, dk1_block, mask=mask_kv1[None, :] & mask_head_dim[:, None])
