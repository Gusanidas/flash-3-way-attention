"""Correctness + performance harness for the H100 kernel vs baseline.

Run from repo root (broker run dir) with:
    PYTHONPATH=src:h100 python3 h100/bench_h100.py
Prints parseable CHECK/PERF lines. Benchmarks both load paths (plain and TMA)
against the baseline Triton kernel.
"""
...
def check_correctness(use_tma):
    ...
    for s, causal, windows_kind in [
        ...
        ok = ok and passed
        print(
            f"CHECK tma={int(use_tma)} s={s} causal={causal} win={windows_kind} "
...
def bench_forward():
    ...
    for s, causal, windows, label in cases:
        ...
        with torch.no_grad():
            ...
            plain_out = flash_trittention_h100(
                q, k1, k2, v1, v2, **kwargs, use_tma=False
            )
            agree_plain = (base_out - plain_out).abs().max().item()
            if agree_plain > 5e-3:
                raise AssertionError(
                    f"H100 plain fp16 output diverged at s={s}: "
                    f"{agree_plain:.3e}"
                )
            try:
                tma_out = flash_trittention_h100(
                    q, k1, k2, v1, v2, **kwargs, use_tma=True
                )
                agree_tma = (base_out - tma_out).abs().max().item()
                if agree_tma > 5e-3:
                    raise AssertionError(
                        f"H100 TMA fp16 output diverged at s={s}: "
                        f"{agree_tma:.3e}"
                    )
                tma_ok = True
                ...
                print(f"TMA-ERROR s={s}: {type(e).__name__}: {e}")
                agree_tma = float("nan")
                tma_ok = False
        ...
        try:
            from h100.fwd_h100 import _tritt_fwd_h100

            best = _tritt_fwd_h100.best_config
        except Exception:
            best = "?"
        print(
            f"PERF s={s} {label} base_ms={base_ms:.3f} "
            f"plain_ms={plain_ms:.3f} ({base_ms / plain_ms:.2f}x) "
            f"tma_ms={tma_ms:.3f} ({base_ms / tma_ms:.2f}x) "
            f"agree_plain={agree_plain:.2e} agree_tma={agree_tma:.2e} cfg=[{best}]"
        )


if __name__ == "__main__":
    print(f"gpu={torch.cuda.get_device_name(0)}")
    ok = check_correctness(use_tma=False)
    ok = check_correctness(use_tma=True) and ok
    if not ok:
